...
- [x] **Stage 3 — Torch profiler pass.** Done 2026-09-09. Both engines profiled with the PyTorch profiler at the same rate=24 / 480-prompt / seed=999 load as Stage 2 (vLLM: `--profiler-config '{"profiler": "torch", "torch_profiler_dir": "<path>"}'` on the server + `vllm bench serve --profile` to bracket start/stop; SGLang: `SGLANG_TORCH_PROFILER_DIR=<path>`). Traces: `traces/{vllm,sglang}_torch_profile/` (Git LFS). Parsed with `bench/parse_torch_profile.py`, raw output in `results/stage3/{vllm,sglang}_kernel_summary.txt`.

  **Methodology note — first vLLM capture was corrupted, not just re-fetched.** The first pass's two `.pt.trace.json.gz` files both failed `gzip -t` (`unexpected end of file`). Root-caused via the server log: `"Stopping profiler..."` fired, but the log went quiet ~1 minute later while the export was still writing (the trace's own async writer hadn't finished — `python_function`-level tracing alone produced 2.7M+ CPU-side events before the writer ever reached the GPU-kernel section), and the file mtimes were a few minutes after that. Something (an earlier cleanup step) killed the server process before the export finished flushing, truncating the gzip stream mid-write — confirmed not a disk/OOM issue (77GB free, no OOM-kills in `dmesg`). Fixed by relaunching vLLM, re-running the identical load with `--profile`, and this time polling `gzip -t` on the output files until they passed *before* touching the server again. Also wrote a truncation-tolerant fallback into the parser itself (`load_events_tolerant()` in `bench/parse_torch_profile.py`) that salvages complete JSON events up to wherever a stream cuts off — kept as a defensive fallback, not needed once the recapture succeeded cleanly.

  **Important scope difference from Stage 2, not a discrepancy:** Stage 2's nsys capture bracketed a short, tightly-bounded *steady-state* window (`nsys start`/`stop` around a slice of the run). Stage 3's torch profiler instead brackets the *entire* 480-prompt run start-to-finish (ramp-up + steady-state + decode tail), so raw totals (launch counts, busy-seconds) aren't directly comparable between the two stages — the op-level *composition* (percentages, dominant-kernel identity, relative attention-vs-GEMM share) is what's meant to cross-check.

  **vLLM: 29.073s total kernel time, 163,731 launches, 48 distinct kernels.**
  - GEMM kernels (all `ampere_bf16_s16816gemm*` shapes, one shape dominant): **68.7%** combined, largest single shape (`...256x128...`) alone at 61.9% (540.96μs avg/launch, 33,256 launches) — same kernel Stage 2 found dominant at 67.3% in its narrower window.
  - FlashAttention split-KV (two KV-length shapes): **26.6%** combined (970.36μs avg/launch on the larger shape) — same kernel family Stage 2 found at 26.5%.
  - Together, **95.3%** of GPU time in these two kernel families — matches Stage 2's "94% in two kernel types" closely enough to validate both capture methods are measuring the same thing.

  **SGLang: 16.278s total kernel time, 318,216 launches, 58 distinct kernels.**
  - GEMM kernels (`ampere_bf16_s16816gemm*`, multiple shapes, most spread across a long tail): **83.6%** combined, largest single shape (`...256x128...`) at 56.2% (266.34μs avg/launch, 34,353 launches).
  - FlashInfer attention kernels (`BatchPrefillWithPagedKVCache`, `BatchPrefillWithRaggedKVCache`, `MergeStates` variants): **9.6%** combined, largest shape at 84.28μs avg/launch.
  - Remaining ~5.7% is RMSNorm/SiLU/RoPE/KV-cache-write elementwise kernels.
  - Not directly comparable to Stage 2's "no dominant kernel, GPU barely busy" finding (different window, see above) — that finding is about SGLang being *idle more of the time* at rate=24, not about running a fundamentally different kernel mix. Aggregated over the whole run, SGLang's kernel mix is GEMM-dominated too, just like vLLM's.

  **The new evidence Stage 3 adds, pointed at Stage 4:**
  1. **Attention-kernel share of total GPU time differs ~2.8x**: 26.6% (vLLM, FlashAttention split-KV) vs. 9.6% (SGLang, FlashInfer). This is a real op-level signal, not just an artifact of vLLM being busier overall — it's a *share* of each engine's own kernel time.
  2. **Per-launch latency on the dominant attention kernel differs ~11.5x**: 970μs (vLLM) vs. 84μs (SGLang), for kernels serving a comparable KV-cache access pattern. Directly supports hypothesis (a) from Stage 2 — FlashAttention's split-KV kernel looks intrinsically more expensive here, not just more frequently invoked.
  3. **GEMM per-launch latency is also ~2x higher for vLLM** (540.96μs vs. 266.34μs) despite near-identical launch counts (33,256 vs. 34,353) — consistent with hypothesis (b): at rate=24, vLLM is well past its Stage-1 knee (~16-20 req/s) and backlogged, while SGLang is comfortably before its own knee (~40-48 req/s); backlog-driven batch composition can produce larger, less efficient GEMM shapes even on the *same* kernel.
  4. **Conclusion: both Stage 2 hypotheses look partially true, not either/or.** The attention-kernel choice looks intrinsically less efficient at the op level (finding 2), *and* vLLM's own GEMM work is running less efficiently under backlog (finding 3). Stage 4's FlashInfer-for-vLLM experiment isolates just the first component — if throughput improves but the ~2.4x ceiling gap doesn't fully close, that's itself evidence the scheduling/backlog effect (hypothesis b) is also contributing, not a failed experiment.

- [x] **Stage 4 — Isolate the cause.** Done 2026-09-09. Experiment: force vLLM onto the FlashInfer attention backend (SGLang already defaults to it — no change needed there) and re-run the identical Stage 1 fine-sweep methodology (same rate grid, per-rate seeds, `ulimit -n 65536`, 512in/128out), changing only that one variable. Results: `results/stage4_flashinfer/vllm/`.

  **Methodology note — the toggle mechanism changed with the 0.28.0 upgrade.** `VLLM_ATTENTION_BACKEND=FLASHINFER` (confirmed toggleable in the Stage 2 write-up, on the pre-upgrade version) silently no-ops on 0.28.0 — the server logs `WARNING: Unknown vLLM environment variable detected: VLLM_ATTENTION_BACKEND` and quietly keeps using `FLASH_ATTN`. It would have been easy to run the whole sweep against an unchanged backend without noticing. Caught by checking the server log for the actual backend-selection line before trusting the run, not just assuming the flag took effect. 0.28.0 moved attention-backend selection to a CLI flag: `vllm serve ... --attention-backend FLASHINFER` (confirmed via `vllm serve --help=AttentionConfig`) — verified via the log's `Using AttentionBackendEnum.FLASHINFER backend.` line before running the sweep.

  **Correctness check before trusting the throughput numbers**: every rate point generated exactly 128.0 avg output tokens/request (512in/128out fixed shape, temperature 0) with zero failed requests — no early-EOS truncation or backend-induced output degradation that could artificially inflate throughput.

  | rate | vLLM+FlashInfer throughput | vLLM+FlashInfer p99 TTFT | (vs. original vLLM+FlashAttention) | (vs. SGLang) |
  |---|---|---|---|---|
  | 8 | 7.78 | 0.17s | 7.79 / 0.18s | 7.45 / 0.11s |
  | 12 | 11.67 | 0.23s | 11.68 / 0.23s | 11.43 / 0.11s |
  | 16 | 15.52 | 0.30s | 15.35 / 0.50s | 14.91 / 0.13s |
  | 20 | 18.76 | 2.21s | 16.23 / 11.07s | 19.21 / 0.16s |
  | 24 | 20.00 | 8.95s | 16.63 / 23.56s | 23.52 / 0.18s |
  | 32 | 20.77 | 29.46s | 17.26 / 48.12s | 30.21 / 0.26s |
  | 40 | 20.81 | 52.52s | 17.46 / 74.41s | 35.13 / 0.52s |
  | 48 | 20.83 | 75.17s | 17.52 / 101.02s | 39.33 / 4.10s |
  | 56 | 20.88 | 95.67s | 17.52 / 126.09s | 41.58 / 10.43s |
  | 64 | 20.95 | 118.66s | 17.59 / 153.31s | 43.06 / 18.27s |

  **Result: the gap partially closes, it doesn't disappear.** Switching vLLM's attention kernel alone raised its throughput ceiling from ~17.5 req/s (converged by rate 32-64 on FlashAttention) to ~20.8-21.0 req/s (converged by rate 32-64 on FlashInfer) — a real **+19% ceiling increase** from one flag, with the knee itself pushed out (rate 20's p99 TTFT drops from 11.07s to 2.21s) and softened into two smaller steps instead of one sharp cliff. But SGLang's ceiling (43.06 req/s) is untouched by this experiment (it was already on FlashInfer) — so the vLLM/SGLang gap moves from **2.45x → 2.05x**, closing roughly **27% of the total gap**, not all of it.

  **Interpretation, tying back to Stage 2-3's two hypotheses:** this is exactly the outcome flagged as possible when Stage 3 was written — partial closure is itself evidence that *both* hypotheses were real, not that the experiment failed. Hypothesis (a) — FlashAttention's split-KV kernel being intrinsically less efficient than FlashInfer's — accounts for a meaningful, measurable slice of the gap (the ~19% ceiling gain, consistent with Stage 3's finding that vLLM's attention-kernel time share and per-launch cost were both disproportionately high). The remaining ~2.05x gap that FlashInfer alone can't close points at hypothesis (b) — scheduling/backlog-driven batch inefficiency — or a structural difference in the engines beyond the attention kernel (e.g. SGLang's RadixAttention prefix-cache-aware scheduler, continuous-batching policy differences) that Stage 4's single-variable swap wasn't designed to isolate. That's out of scope for this project's stated plan (Stage 4 picks *one* variable, not an open-ended search) — worth naming as a "what's next" in the write-up rather than chasing further here.

- [x] **Stage 5 — Write-up & dashboard.** Done 2026-09-09 (deployment pending). The gap (Stage 1), the hypothesis (Stages 2-3), the isolating experiment (Stage 4), the result — `write-up.md`, numbers and profiler evidence first, narrative second.
